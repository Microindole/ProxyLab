...
from proxy_traffic_lab.capture import segmented
...
def test_format_bytes_uses_binary_units() -> None:
    ...
    assert format_bytes(32 * 1024) == "32.00 KiB"
...
def test_segmented_capture_rotates_only_after_idle(monkeypatch) -> None:
    ...
    monkeypatch.setattr(segmented, "capture_segment", fake_segment)
    monkeypatch.setattr(
        segmented,
    ...
    sessions = segmented.run_segmented_capture(
        case=case,
...
def test_segmented_capture_does_not_split_an_active_flow(monkeypatch) -> None:
    ...
    monkeypatch.setattr(segmented, "capture_segment", fake_segment)
    monkeypatch.setattr(segmented, "ensure_sudo_credentials", lambda: None)
    case = SimpleNamespace(enabled=True, id="case-05", outer_transport="tcp")
    sessions = segmented.run_segmented_capture(
        case=case,
        server_ip="203.0.113.10",
